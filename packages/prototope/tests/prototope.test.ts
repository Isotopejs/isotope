...
import * as subSelectors from "../src/sub-selectors";
import * as utils from "../src/utils";
import { createMatcher, createWrapper } from "./utils";
import { Util } from "../src/declarations";
...
const processUtil = (util: TestUtil | TestUtil[]): Util | Util[] => {
	...
	if (typeof util === "object") {
		return (utils[util.name] as (parameter?: string | number) => Util)(util.parameter);
	}
...
describe("Create proper CSS rule", () => {
	describe.each([
		...
		test.each(testCases as any)("%#", (testCase) => {
			...
			const css = minifier.minify(wrapper.getCSS()).styles;
